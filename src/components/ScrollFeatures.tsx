...
export default function ScrollFeatures({ isRtl }: ScrollFeaturesProps) {
  const [showScrollTop, setShowScrollTop] = useState(false);
  const [scrollProgress, setScrollProgress] = useState(0);

  useEffect(() => {
    // Performance Optimization: Throttled Scroll Listener
    // Isolating this in a separate component prevents the entire Home page from re-rendering on scroll
    let ticking = false;
    const handleScroll = () => {
      if (!ticking) {
