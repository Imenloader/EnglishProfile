...
export default function AdminDashboard() {
  ...
  const handleExportExcel = async () => {
    try {
      ...
      // ⚡ Bolt Optimization: Pre-compute answers grouped by lead_id to achieve O(1) lookups instead of O(N^2) filtering
      ...
      const matrixData = filteredLeads.map((l: any) => {
        ...
        // ⚡ Bolt Optimization: Pre-compute map of answers by question text to prevent O(N) .find inside loop
        const studentAnswerByQuestion = new Map();
        studentAnswers.forEach((a: any) => studentAnswerByQuestion.set(a.question_text, a));
        ...
        uniqueQuestions.forEach((q: any) => {
          const ans = studentAnswerByQuestion.get(q);
      ...
      // 3. DETAILED ANSWERS
      // ⚡ Bolt Optimization: Use a Set for filtered lead IDs to turn O(L) .some check into O(1) .has check
