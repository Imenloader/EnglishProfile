...
export default function AdminDashboard() {
  ...
  const handleExportExcel = async () => {
    try {
      ...
      // Pre-compute Hash Maps and Sets for O(1) lookups to prevent O(N^2) bottlenecks
      // 2. STUDENT-QUESTION MATRIX
      const uniqueQuestions = Array.from(new Set((answersData || []).map((a: any) => a.question_text)));

      // Group answers by lead_id and then by question_text for O(1) lookup
      const answersByLeadAndQuestion = new Map<string, Map<string, any>>();
      ...
      const matrixData = filteredLeads.map((l: any) => {
        const leadAnswers = answersByLeadAndQuestion.get(l.id) || new Map();
        const row: any = {
          'Student Name': l.name,
          'Phone': l.phone || 'N/A',
          'Company': l.company || 'N/A',
      ...
      // 3. DETAILED ANSWERS
      // Use Set for O(1) lookups instead of .some() in an array
