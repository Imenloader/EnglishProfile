...
export default function AdminDashboard() {
  ...
  const handleExportExcel = async () => {
    try {
      ...
      // Optimization: Pre-compute a Map grouping answers by lead_id for O(1) lookups
      ...
      const matrixData = filteredLeads.map((l: any) => {
        const studentAnswers = answersByLeadId.get(l.id) || new Map();
        const row: any = {
          'Student Name': l.name,
          'Phone': l.phone || 'N/A',
          'Company': l.company && l.company.includes('Prefers:') ? l.company.split('Prefers:')[0].replace('(', '').trim() || 'N/A' : l.company || 'N/A',
        ...
        uniqueQuestions.forEach((q: any) => {
          const ans = studentAnswers.get(q as string);
      ...
      // 3. DETAILED ANSWERS
      // Optimization: Create a Set of filteredLeadIds to optimize detailed answers filtering (O(1) vs O(N))
