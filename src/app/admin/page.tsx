...
export default function AdminDashboard() {
  ...
  const handleExportExcel = async () => {
    try {
      ...
      // ⚡ Bolt: Pre-compute nested Map of answers by lead ID and question text
      // Eliminates O(M) .filter() and .find() inside O(N) map, reducing O(N * M^2) to O(N + M)
      ...
      // 3. DETAILED ANSWERS
      // ⚡ Bolt: Pre-compute Set of filtered lead IDs to achieve O(1) lookup
      // Reduces O(N * M) filtering operation to O(N)
