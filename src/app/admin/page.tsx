...
export default function AdminDashboard() {
  ...
  const handleExportExcel = async () => {
    try {
      ...
      // Pre-compute O(1) lookups for leads and answers to prevent O(N²) iterations
      // Create a set of filtered lead IDs for fast lookups
      const filteredLeadIds = new Set(filteredLeads.map((l: any) => l.id));

      // Group answers by lead_id, and then map each question_text to the answer for O(1) matrix building
      ...
      // 3. DETAILED ANSWERS
      const detailedAnswers = (answersData || [])
        .filter((a: any) => filteredLeadIds.has(a.lead_id))
        .map((a: any) => ({
