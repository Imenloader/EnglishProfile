...
## 2026-05-24 - HTML Img Tags vs Next.js Image
**Learning:** Standard HTML `<img>` tags were being used for logos and partner images, resulting in unoptimized static assets being loaded directly. Using Next.js `<Image>` significantly improves performance via automatic optimization and lazy loading.
**Action:** Always prefer the `<Image>` component from `next/image` over raw `<img>` tags when working within a Next.js environment to reduce initial payload and LCP metrics.
