...
## 2026-05-28 - Optimize Excel Export Array Operations
**Learning:** The handleExportExcel function used nested array iterations (.some, .filter, .find inside .map) resulting in O(N^2) complexity, causing significant UI freezing when generating large excel exports.
**Action:** Use pre-computed Maps for grouping and Sets for filtering data before nested iterations to achieve O(N) complexity for related data processing.
