...
## 2024-05-16 - O(N^2) Bottlenecks in Data Export
**Learning:** During Excel export generation in `AdminDashboard`, there were multiple instances of `Array.filter()`, `Array.some()`, and `Array.find()` inside `Array.map()` operations when cross-referencing `leads` and `answers`. These $O(N^2)$ operations cause significant frontend freezing when exporting large datasets.
**Action:** When performing data aggregations or exports involving multiple related datasets, always pre-compute Hash Maps (`Map`) or Hash Sets (`Set`) for cross-referencing. This reduces lookup time to $O(1)$ and overall complexity to $O(N)$.
